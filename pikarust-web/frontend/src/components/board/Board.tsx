...
export function Board({ position, onSquareClick, selectedSquare, validMoves = [], lastMove, inCheck, flipped = false }: BoardProps) {
  ...
  function handleClick(e: React.MouseEvent<SVGSVGElement>) {
    ...
    let x = ((e.clientX - rect.left) / rect.width) * w - padding;
    let y = ((e.clientY - rect.top) / rect.height) * h - padding;
  ...
  return (
    ...
    >
      ...
      <rect x={-padding} y={-padding} width={w} height={h} fill="var(--color-surface)" />

      <g transform={flipped ? 'rotate(180, 4, 4.5)' : undefined}>
        <BoardGrid flipped={flipped} />
